...
Var msg:Message;
Begin
  Assert (MultiSetCount(i:Net, true) < NetMax) "Too many messages";
  msg.MType := M;
  msg.Source := Src;
  msg.Destination := Dst;
  msg.Aux := Aux;
...
Choose M_Index: Net Do
  ...
  Rule
      -- is it for a remote processor?
      Ismember(Net[M_Index].Destination,Proc)
    ==>
      Alias msg:Net[M_Index] Do
...
Choose M_Index: Net Do
  ...
  Rule
    -- is it for a home processor?
      Ismember(Net[M_Index].Destination,Home)
    ==>
      Alias msg:Net[M_Index] Do
      Alias n:msg.Destination Do
      Alias me:Homes[n] Do
      ...
      Forall home:Home Do
  	  ...
  	  !(  Procs[n1].Cache[home][addr].State = S
  	    ...
  	    & MultiSetCount(i:Net,
  	           ( (  (Net[i].MType = Invalidate)
  	             & (Net[i].Source = home)
  	             & (Net[i].Destination = n1))
  		   | (  (Net[i].MType = Update)
  	             & (Net[i].Destination = home)
  		     & (Net[i].Value = Procs[n1].Cache[home][addr].Value))
  		   | (  (Net[i].MType = Write_Back)
  	             & (Net[i].Destination = home)
  		     & (Net[i].Value = Procs[n1].Cache[home][addr].Value)))
		     ) = 0
  	      )
...
multiset network
 	...
 	* The size of each state is 177 bits (rounded up to 23 bytes).

	BFS -sym2
	7909 states,34853 rules fired in 594.96 s
	721 states max in the queue.

	5 processors 1 data values

        7183 states, 32606 rules fired in 115.35s.
        * The size of each state is 191 bits (rounded up to 24 bytes).


	7 processors 2 data values

	34348 states, 184516 rules fired in 14948.77s
        * The size of each state is 292 bits (rounded up to 37 bytes).

Release 2.9S (Sparc 20, cabbage.stanford.edu)

    5 processes 2 values
       * The size of each state is 640 bits (rounded up to 80 bytes).
     -b* The size of each state is 253 bits (rounded up to 32 bytes).

        13738 states, 65357 rules fired in 42.36s.
     -b 13738 states, 65357 rules fired in 57.95s.
     -c 13738 states, 65357 rules fired in 36.83s.
