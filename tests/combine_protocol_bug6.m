...
-- /*----bombCnt1 rule ----*/
-- rule "bombCnt1 update"
-- bombCnt1 < BL ==> bombCnt1 := bombCnt1 + 1;
-- endrule;
-- /*----end bombCnt1 rule ----*/
...
-- /*----bombCnt2 rule ----*/
-- rule "bombCnt2 update"
-- bombCnt2 < BL ==> bombCnt2 := bombCnt2 + 1;
-- endrule;
-- /*----end bombCnt2 rule ----*/
...
PER WALK REPORTS:
       report: NONE 	(default)
  history len: 4 	(config when generating with romp)
  print state: YES 	(default)
...
PROPERTY(S) VIOLATED (n=2 |n|=224):
  -(1) invariant "flag1_2": (x121)
    -[w#2] seed=504286273 start-id=5 depth=0 t=0.25ms (Δt=0.25ms)
            last-rule={startstate "Init" quantifiers(d:=Datas_3)}
    -[w#3] seed=2038161738 start-id=3 depth=0 t=0.12ms (Δt=0.12ms)
            last-rule={startstate "Init" quantifiers(d:=Datas_1)}
    -[w#4] seed=1650424188 start-id=3 depth=0 t=0.10ms (Δt=0.10ms)
            last-rule={startstate "Init" quantifiers(d:=Datas_1)}
    -[..] ... 118 more ...
  -(2) invariant "_Invld___Gblock_WB": (x103)
    -[w#0] seed=1559321741 start-id=1 depth=0 t=0.80ms (Δt=0.80ms)
            last-rule={startstate "bombCnt1_2_and_flag1_2" quantifiers(d:=Datas_2)}
    -[w#1] seed=1279319282 start-id=2 depth=0 t=0.17ms (Δt=0.17ms)
            last-rule={startstate "bombCnt1_2_and_flag1_2" quantifiers(d:=Datas_3)}
    -[w#7] seed=959239224 start-id=2 depth=0 t=5022ns (Δt=5169.987.679ns)
            last-rule={startstate "bombCnt1_2_and_flag1_2" quantifiers(d:=Datas_3)}
    -[..] ... 100 more ...
...
RW SUMMARY:
         issues found: n=2  (|n|=224)
  total rules applied: 0
    avg rules applied: 0
      total walk time: t=3.154ms (Δt=3.199ms)
