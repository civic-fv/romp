/*
	Generated code for a romp "parallel random walker" verification tool based off of the Murphi Model described in:
		Original Murphi Model: n_peterson.bug1.m
...
#define __model__filepath "/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.bug1.m"


#define __model__filepath_contains_space (true)


#define __model__filename "n_peterson.bug1.m"
...
namespace __type__ {
  ...
  const __typeof__(((int)(4))) MSL = ((int)(4)); // must be 2<=MSL<=LHL to be a valid test for subseq
...
#else
  template<class O> const ::romp::stream_void& lh_r_to_json(::romp::ojstream<O>& json, const lh_r& val) noexcept { json << "{\"$type\":\"range-value\",\"type\":\"lh_r\",\"min\":" << (VALUE_C(0)) << ",\"max\":" << (VALUE_C(6)) << ",\"value\":" << ((range_t)val) << "}"; return ::romp::S_VOID;}
#endif
...
#else
  template<class O> const ::romp::stream_void& bug_r_to_json(::romp::ojstream<O>& json, const bug_r& val) noexcept { json << "{\"$type\":\"range-value\",\"type\":\"bug_r\",\"min\":" << (VALUE_C(0)) << ",\"max\":" << (VALUE_C(13)) << ",\"value\":" << ((range_t)val) << "}"; return ::romp::S_VOID;}
#endif
...
#else
  template<class O> const ::romp::stream_void& anon0x4_t_to_json(::romp::ojstream<O>& json, const anon0x4_t& val) noexcept {json << "{\"$type\":\"array-value\",\"type\":\"Array[lh_r] of pid\",\"size\":" << val.size() << ",\"lower-bound\":" << VALUE_C(0) << ",\"upper-bound\":" << VALUE_C(6) << ",\"elements\":["; std::string sep; for (size_t i=(0ul); i<val.size(); ++i) { json << sep << pid_to_json(json,val.data[i]); sep = ","; } json << "]}"; return ::romp::S_VOID;}
#endif
  ...
  typedef struct { ::__type__::pid data[7]; size_t size() const {return (7ul);} } anon0x5_t;
#ifdef __ROMP__SIMPLE_TRACE
...
#define _ROMP_STATESPACE_COUNT_str "168202007442751287527018229104809869312"
...
#define _ROMP_FUNCTS_LEN (2ul)
  ...
  const ::romp::FunctInfo FUNCT_INFOS[_ROMP_FUNCTS_LEN] = {{"bug",{{148,1},{161,5}},"{\"$type\":\"function\",\"return-type\":\"pid\",\"label\":\"bug\",\"params\":[{\"$type\":\"param\",\"id\":\"i\",\"type\":\"bug_r\"}],\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.bug1.m\",\"start\":[148,1],\"end\":[161,5]}}","function bug(var i: bug_r) : pid;"},{"print_bug",{{163,1},{171,5}},"{\"$type\":\"procedure\",\"label\":\"print_bug\",\"params\":[{\"$type\":\"param\",\"id\":\"tmp\",\"type\":\"boolean\"}],\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.bug1.m\",\"start\":[163,1],\"end\":[171,5]}}","procedure print_bug(var tmp: boolean);"},};
...
#define _ROMP_ERRORS_LEN (2ul)
  ...
  const ::romp::MErrorInfo ERROR_INFOS[_ROMP_ERRORS_LEN] = {{"CONFIG ERROR: `MSL` needs to >= 2 to be a proper subseq !!",{{192,10},{192,76}},0,"{\"$type\":\"error-statement\",\"label\":\"CONFIG ERROR: `MSL` needs to >= 2 to be a proper subseq !!\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.bug1.m\",\"start\":[192,10],\"end\":[192,76]}"},{"CONFIG ERROR: `MSL` needs to <=`LHL` for a subsequence to be found !!",{{195,10},{195,87}},0,"{\"$type\":\"error-statement\",\"label\":\"CONFIG ERROR: `MSL` needs to <=`LHL` for a subsequence to be found !!\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.bug1.m\",\"start\":[195,10],\"end\":[195,87]}"},};
...
#define _ROMP_STARTSTATE_INFO_LEN (1ul)
  ...
  const ::romp::StartStateInfo STARTSTATE_INFOS[_ROMP_STARTSTATE_INFO_LEN] = {{"init",{{174,1},{198,4}},"{\"$type\":\"startstate\",\"label\":\"init\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.bug1.m\",\"start\":[174,1],\"end\":[198,4]}"},};
...
#define _ROMP_RULESETS_LEN (5ul)
  ...
  const ::romp::RuleInfo RULE_SET_INFOS[_ROMP_RULESETS_LEN] = {{"L0_execute_inc_j_and_while",{{95,3},{100,6}},"{\"$type\":\"rule\",\"expr\":\"(P[i] = L0)\",\"label\":\"L0_execute_inc_j_and_while\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.bug1.m\",\"start\":[95,3],\"end\":[100,6]}"},{"L1_execute_assign_Qi_j",{{102,3},{107,6}},"{\"$type\":\"rule\",\"expr\":\"(P[i] = L1)\",\"label\":\"L1_execute_assign_Qi_j\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.bug1.m\",\"start\":[102,3],\"end\":[107,6]}"},{"L2_execute_assign_TURNj_i",{{109,3},{114,6}},"{\"$type\":\"rule\",\"expr\":\"(P[i] = L2)\",\"label\":\"L2_execute_assign_TURNj_i\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.bug1.m\",\"start\":[109,3],\"end\":[114,6]}"},{"L3_execute_wait_until",{{116,3},{132,6}},"{\"$type\":\"rule\",\"expr\":\"(P[i] = L3)\",\"label\":\"L3_execute_wait_until\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.bug1.m\",\"start\":[116,3],\"end\":[132,6]}"},{"L4_execute_critical_and_assign_Qi_1",{{134,3},{143,6}},"{\"$type\":\"rule\",\"expr\":\"(P[i] = L4)\",\"label\":\"L4_execute_critical_and_assign_Qi_1\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.bug1.m\",\"start\":[134,3],\"end\":[143,6]}"},};
