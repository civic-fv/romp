...
namespace __type__ {
  ...
  typedef struct { ::__type__::val_t data[5]; size_t size() const {return (5ul);} } arr_t;
#ifdef __ROMP__SIMPLE_TRACE
  ...
  template<class O> const ::romp::stream_void& bool_t_to_json(::romp::ojstream<O>& json, const bool_t& val) noexceptjson << boolean_to_json(json,((boolean)val)); return ::romp::S_VOID;}
...
#endif
const ::romp::stream_void& __Model___to_str(::romp::ostream_p& out, const __Model__& val) noexcept { using namespace ::__type__; out << "{\n" << out.indent() << out.indentation() << "i " << ((::romp::OPTIONS.result_show_type) ? ": ind_t =" : ":=") << ' ' << ind_t_to_str(out,val.i) << '\n' << out.indentation() << "j " << ((::romp::OPTIONS.result_show_type) ? ": ind_t =" : ":=") << ' ' << ind_t_to_str(out,val.j) << '\n' << out.indentation() << "a " << ((::romp::OPTIONS.result_show_type) ? ": arr_t =" : ":=") << ' ' << arr_t_to_str(out,val.a) << '\n' << out.dedent()<< out.indentation() << '}'; return ::romp::S_VOID;}
}
namespace romp {
#ifdef __ROMP__SIMPLE_TRACE
