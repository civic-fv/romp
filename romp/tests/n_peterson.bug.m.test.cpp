...
namespace __type__ {
  ...
  const __typeof__(((int)(2))) MSL = ((int)(2)); // the minimum length of the bug sub sequence
...
#else
  template<class O> const ::romp::stream_void& b_ind_t_to_json(::romp::ojstream<O>& json, const b_ind_t& val) noexcept { json << "{\"$type\":\"range-value\",\"type\":\"b_ind_t\",\"min\":" << (VALUE_C(0)) << ",\"max\":" << (VALUE_C(7)) << ",\"value\":" << ((range_t)val) << "}"; return ::romp::S_VOID;}
#endif
...
#else
  template<class O> const ::romp::stream_void& lh_t_to_json(::romp::ojstream<O>& json, const lh_t& val) noexcept { json << "{\"$type\":\"range-value\",\"type\":\"lh_t\",\"min\":" << (VALUE_C(0)) << ",\"max\":" << (VALUE_C(3)) << ",\"value\":" << ((range_t)val) << "}"; return ::romp::S_VOID;}
#endif
...
#else
  template<class O> const ::romp::stream_void& anon0x4_t_to_json(::romp::ojstream<O>& json, const anon0x4_t& val) noexcept {json << "{\"$type\":\"array-value\",\"type\":\"Array[lh_t] of pid\",\"size\":" << val.size() << ",\"lower-bound\":" << VALUE_C(0) << ",\"upper-bound\":" << VALUE_C(3) << ",\"elements\":["; std::string sep; for (size_t i=(0ul); i<val.size(); ++i) { json << sep << pid_to_json(json,val.data[i]); sep = ","; } json << "]}"; return ::romp::S_VOID;}
#endif
  const ::romp::stream_void& anon0x4_t_to_str(::romp::ostream_p& out, const anon0x4_t& val) noexcept { out << "[\n" << out.indent(); for (size_t i=0; i<val.size(); ++i) { out << out.indentation() << "[" << (i + (VALUE_C(0))) << "]: "<< pid_to_str(out,val.data[i]) << '\n'; } out << out.dedent() << out.indentation() << ']'; return ::romp::S_VOID;}

  typedef struct { ::__type__::pid data[8]; size_t size() const {return (8ul);} } anon0x5_t; // where we store first locking order to use as bug seq
#ifdef __ROMP__SIMPLE_TRACE
...
#define _ROMP_STATESPACE_COUNT_str "18160335421875000000000000000"
