...
namespace __type__ {
  ...
  const __typeof__(((int)(6))) N = ((int)(6)); // the number of processes
  const __typeof__((N)) LHL = (N); // the size of the history buffer
  const __typeof__(((LHL) * ((int)(2)))) BL = ((LHL) * ((int)(2))); // the used size of the "bug array"
  const __typeof__(((int)(3))) MSL = ((int)(3)); // must be 2<=MSL<=LHL to be a valid test for subseq
...
#else
  template<class O> const ::romp::stream_void& pid_to_json(::romp::ojstream<O>& json, const pid& val) noexcept { json << "{\"$type\":\"scalarset-value\",\"type\":\"pid\",\"bound\":6,\"value\":" << ((scalarset_t)val) << "}"; return ::romp::S_VOID;}
#endif
...
#else
  template<class O> const ::romp::stream_void& pid_r_to_json(::romp::ojstream<O>& json, const pid_r& val) noexcept { json << "{\"$type\":\"range-value\",\"type\":\"pid_r\",\"min\":" << (VALUE_C(0)) << ",\"max\":" << (VALUE_C(5)) << ",\"value\":" << ((range_t)val) << "}"; return ::romp::S_VOID;}
#endif
  const ::romp::stream_void& pid_r_to_str(::romp::ostream_p& out, const pid_r& val) noexcept { out << ((range_t)val); return ::romp::S_VOID;}

  typedef range_t lh_r;
#ifdef __ROMP__SIMPLE_TRACE
  template<class O> const ::romp::stream_void& lh_r_to_json(::romp::ojstream<O>& json, const lh_r& val) noexcept { json << val; return ::romp::S_VOID; }
#else
  template<class O> const ::romp::stream_void& lh_r_to_json(::romp::ojstream<O>& json, const lh_r& val) noexcept { json << "{\"$type\":\"range-value\",\"type\":\"lh_r\",\"min\":" << (VALUE_C(0)) << ",\"max\":" << (VALUE_C(5)) << ",\"value\":" << ((range_t)val) << "}"; return ::romp::S_VOID;}
#endif
  const ::romp::stream_void& lh_r_to_str(::romp::ostream_p& out, const lh_r& val) noexcept { out << ((range_t)val); return ::romp::S_VOID;}
...
#else
  template<class O> const ::romp::stream_void& bug_r_to_json(::romp::ojstream<O>& json, const bug_r& val) noexcept { json << "{\"$type\":\"range-value\",\"type\":\"bug_r\",\"min\":" << (VALUE_C(0)) << ",\"max\":" << (VALUE_C(11)) << ",\"value\":" << ((range_t)val) << "}"; return ::romp::S_VOID;}
#endif
...
#else
  template<class O> const ::romp::stream_void& priority_to_json(::romp::ojstream<O>& json, const priority& val) noexcept { json << "{\"$type\":\"range-value\",\"type\":\"priority\",\"min\":" << (VALUE_C(0)) << ",\"max\":" << (VALUE_C(6)) << ",\"value\":" << ((range_t)val) << "}"; return ::romp::S_VOID;}
#endif
  ...
  typedef struct { ::__type__::label_t data[6]; size_t size() const {return (6ul);} } anon0x0_t; // stores the current state of each process
#ifdef __ROMP__SIMPLE_TRACE
  ...
  typedef struct { ::__type__::priority data[6]; size_t size() const {return (6ul);} } anon0x1_t; // stores what priority each process has
#ifdef __ROMP__SIMPLE_TRACE
  ...
  typedef struct { ::__type__::pid data[7]; size_t size() const {return (7ul);} } anon0x2_t; // maps each priority to a process
#ifdef __ROMP__SIMPLE_TRACE
  ...
  typedef struct { ::__type__::priority data[6]; size_t size() const {return (6ul);} } anon0x3_t; // maps each process to it's current priority
#ifdef __ROMP__SIMPLE_TRACE
  ...
  typedef struct { ::__type__::pid data[6]; size_t size() const {return (6ul);} } anon0x4_t;
#ifdef __ROMP__SIMPLE_TRACE
...
#else
  template<class O> const ::romp::stream_void& m_int_to_json(::romp::ojstream<O>& json, const m_int& val) noexcept { json << "{\"$type\":\"range-value\",\"type\":\"__UNKNOWN_TYPE_REPR__\",\"min\":" << (VALUE_C(-9223372036854775807)) << ",\"max\":" << (VALUE_C(9223372036854775807)) << ",\"value\":" << ((range_t)val) << "}"; return ::romp::S_VOID;}
#endif
  const ::romp::stream_void& m_int_to_str(::romp::ostream_p& out, const m_int& val) noexcept { out << ((range_t)val); return ::romp::S_VOID;}

  typedef struct { ::__type__::pid data[6]; size_t size() const {return (6ul);} } anon0x5_t;
#ifdef __ROMP__SIMPLE_TRACE
...
namespace __model__ {
  ...
  struct __Model__ {
    ...
    ::__type__::lh_r lh_size; // start and end of circ buffer
    ...
    ::__type__::pid bug(const ::__type__::bug_r i) const  {
      ...
      try {
        ...
        // the used size of the "bug array"
        // must be 2<=MSL<=LHL to be a valid test for subseq
        ...
        do {
          for (int p = 0; p < (N); p++) {
            do {
              for (int tmp = (p); tmp <= (p); tmp += 1) {
                ((pids).data[((tmp)) - int_VALUE_C(0)]) = (p);
              }
            } while (0);
          }
        } while (0);
        ...
        (fact) = ((i) + ((int)(3)));
        do {
          for (int j = ((int)(2)); j <= ((i) + ((int)(2))); j += 1) {
            (fact) = ((fact) * (j));
          }
        } while (0);
        (sum) = ((i) + ((int)(1)));
        do {
          for (int j = ((int)(1)); j <= (i); j += 1) {
            (sum) = ((sum) + (j));
          }
        } while (0);
    ...
    /* ======= Murphi Model StartStates, Rules & Global Properties ====== */
     ...
     void __StartState__init(void){
      ...
      try {
        ...
        ::__model__::__Model__::print_bug((true));
    ...
    bool __Property__injected_hist_subseq_bug(void) const {
      using namespace ::__type__;
      try {
        return this->__rw__->invariant_handler(((!(lh_is_full)) || (!({ bool res_3 = false; for (int l = (MSL); l <= (((BL) < (LHL)) ? (BL) : (LHL)); l += 1) { res_3 |= ({ bool res_4 = false; for (int b_i = ((int)(0)); b_i <= ((BL) - (l)); b_i += 1) { res_4 |= ({ bool res_5 = false; for (int lh_i = ((int)(0)); lh_i <= ((LHL) - (l)); lh_i += 1) { res_5 |= ({ bool res_6 = true; for (int j = ((int)(0)); j <= ((l) - ((int)(1))); j += 1) { res_6&= (::__model__::__Model__::bug(((b_i) + (j))) == ((lock_hist).data[(((lh_i) + (j))) - int_VALUE_C(0)])); } res_6; }); } res_5; }); } res_4; }); } res_3; }))),1u);
...
#define _ROMP_STATESPACE_COUNT_str "122746315735057606344799244255232"
/* the number of functions & procedures in the model */
#define _ROMP_FUNCTS_LEN (2ul)
  /* the info/metadata about the functions/procedures in the model */
  const ::romp::FunctInfo FUNCT_INFOS[_ROMP_FUNCTS_LEN] = {{"bug",{{148,1},{161,5}},"{\"$type\":\"function\",\"return-type\":\"pid\",\"label\":\"bug\",\"params\":[{\"$type\":\"param\",\"id\":\"i\",\"type\":\"bug_r\"}],\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.hard-bug.m\",\"start\":[148,1],\"end\":[161,5]}}","function bug(var i: bug_r) : pid;"},{"print_bug",{{163,1},{171,5}},"{\"$type\":\"procedure\",\"label\":\"print_bug\",\"params\":[{\"$type\":\"param\",\"id\":\"tmp\",\"type\":\"boolean\"}],\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.hard-bug.m\",\"start\":[163,1],\"end\":[171,5]}}","procedure print_bug(var tmp: boolean);"},};
...
#define _ROMP_ERRORS_LEN (2ul)
  ...
  const ::romp::MErrorInfo ERROR_INFOS[_ROMP_ERRORS_LEN] = {{"CONFIG ERROR: `MSL` needs to >= 2 to be a proper subseq !!",{{192,10},{192,76}},0,"{\"$type\":\"error-statement\",\"label\":\"CONFIG ERROR: `MSL` needs to >= 2 to be a proper subseq !!\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.hard-bug.m\",\"start\":[192,10],\"end\":[192,76]}"},{"CONFIG ERROR: `MSL` needs to <=`LHL` for a subsequence to be found !!",{{195,10},{195,87}},0,"{\"$type\":\"error-statement\",\"label\":\"CONFIG ERROR: `MSL` needs to <=`LHL` for a subsequence to be found !!\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.hard-bug.m\",\"start\":[195,10],\"end\":[195,87]}"},};
...
#define _ROMP_LIVENESS_PROP_COUNT (0ul) // the number of property rules (after ruleset expansion) in the model
  ...
  const ::romp::PropertyInfo PROPERTY_INFOS[_ROMP_PROPERTIES_LEN] = {{"only_one_P_has_lock",::romp::PropertyType::INVARIANT,"(!exists i1 : pid do exists i2 : pid do (((i1 != i2) & (P[i1] = L4)) & (P[i2] = L4)) endexists endexists)",0,{{200,1},{208,8}},"{\"$type\":\"property\",\"type\":\"invariant\",\"expr\":\"(!exists i1 : pid do exists i2 : pid do (((i1 != i2) & (P[i1] = L4)) & (P[i2] = L4)) endexists endexists)\",\"label\":\"only_one_P_has_lock\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.hard-bug.m\",\"start\":[200,1],\"end\":[208,8]}"},{"injected_hist_subseq_bug",::romp::PropertyType::INVARIANT,"((!lh_is_full) | (!exists l from MSL to ((BL < LHL) ? BL : LHL) do exists b_i from 0 to (BL - l) do exists lh_i from 0 to (LHL - l) do forall j from 0 to (l - 1) do (bug((b_i + j)) = lock_hist[(lh_i + j)]) endforall endexists endexists endexists))",1,{{211,1},{221,18}},"{\"$type\":\"property\",\"type\":\"invariant\",\"expr\":\"((!lh_is_full) | (!exists l from MSL to ((BL < LHL) ? BL : LHL) do exists b_i from 0 to (BL - l) do exists lh_i from 0 to (LHL - l) do forall j from 0 to (l - 1) do (bug((b_i + j)) = lock_hist[(lh_i + j)]) endforall endexists endexists endexists))\",\"label\":\"injected_hist_subseq_bug\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.hard-bug.m\",\"start\":[211,1],\"end\":[221,18]}"},};
...
#define _ROMP_STARTSTATE_INFO_LEN (1ul)
  ...
  const ::romp::StartStateInfo STARTSTATE_INFOS[_ROMP_STARTSTATE_INFO_LEN] = {{"init",{{174,1},{198,4}},"{\"$type\":\"startstate\",\"label\":\"init\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/n_peterson.hard-bug.m\",\"start\":[174,1],\"end\":[198,4]}"},};
