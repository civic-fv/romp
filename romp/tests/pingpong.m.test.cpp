/*
	Generated code for a romp "parallel random walker" verification tool based off of the Murphi Model described in:
		Original Murphi Model: pingpong.m
...
#define __model__filepath "/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/pingpong.m"


#define __model__filepath_contains_space (true)


#define __model__filename "pingpong.m"
...
#ifdef __ROMP__SIMPLE_TRACE
  template<class O> const ::romp::stream_void& player_t_to_json(::romp::ojstream<O>& json, const player_t& val) noexcept { json << val; return ::romp::S_VOID; }
#else
  template<class O> const ::romp::stream_void& player_t_to_json(::romp::ojstream<O>& json, const player_t& val) noexcept { json << "{\"$type\":\"range-value\",\"type\":\"player_t\",\"min\":" << (VALUE_C(0)) << ",\"max\":" << (VALUE_C(1)) << ",\"value\":" << ((range_t)val) << "}"; return ::romp::S_VOID;}
#endif
  const ::romp::stream_void& player_t_to_str(::romp::ostream_p& out, const player_t& val) noexcept { out << ((range_t)val); return ::romp::S_VOID;}
...
#ifdef __ROMP__SIMPLE_TRACE
  template<class O> const ::romp::stream_void& anon0x0_t_to_json(::romp::ojstream<O>& json, const anon0x0_t& val) noexcept { using namespace ::__type__; json  << boolean_to_json(json,val.hasball) << ',' << boolean_to_json(json,val.gotball); return ::romp::S_VOID; }
#else
  template<class O> const ::romp::stream_void& anon0x0_t_to_json(::romp::ojstream<O>& json, const anon0x0_t& val) noexcept { using namespace ::__type__; json << "{\"$type\":\"record-value\",\"type\":\"anon_record_t\",\"fields\":[" << "{\"$type\":\"kv-pair\",\"key\":\"hasball\",\"value\":" << boolean_to_json(json,val.hasball) << "}," << "{\"$type\":\"kv-pair\",\"key\":\"gotball\",\"value\":" << boolean_to_json(json,val.gotball)<< "}]}"; return ::romp::S_VOID;}
#endif
  const ::romp::stream_void& anon0x0_t_to_str(::romp::ostream_p& out, const anon0x0_t& val) noexcept { using namespace ::__type__; out << "{\n" << out.indent() << out.indentation() << "hasball " << ((::romp::OPTIONS.result_show_type) ? ": __Model__ =" : ":=") << ' ' << boolean_to_str(out,val.hasball) << '\n' << out.indentation() << "gotball " << ((::romp::OPTIONS.result_show_type) ? ": __Model__ =" : ":=") << ' ' << boolean_to_str(out,val.gotball) << '\n' << out.dedent()<< out.indentation() << '}'; return ::romp::S_VOID;}

  typedef struct { ::__type__::anon0x0_t data[2]; constexpr const size_t size() const {return (2ul);} } anon0x1_t;
#ifdef __ROMP__SIMPLE_TRACE
  template<class O> const ::romp::stream_void& anon0x1_t_to_json(::romp::ojstream<O>& json, const anon0x1_t& val) noexcept {std::string sep; for (size_t i=(0ul); i<val.size(); ++i) { json << sep << anon0x0_t_to_json(json,val.data[i]); sep = ","; } return ::romp::S_VOID;}
#else
  template<class O> const ::romp::stream_void& anon0x1_t_to_json(::romp::ojstream<O>& json, const anon0x1_t& val) noexcept {json << "{\"$type\":\"array-value\",\"type\":\"Array[player_t] of anon_record_t\",\"size\":" << val.size() << ",\"lower-bound\":" << VALUE_C(0) << ",\"upper-bound\":" << VALUE_C(1) << ",\"elements\":["; std::string sep; for (size_t i=(0ul); i<val.size(); ++i) { json << sep << anon0x0_t_to_json(json,val.data[i]); sep = ","; } json << "]}"; return ::romp::S_VOID;}
#endif
  const ::romp::stream_void& anon0x1_t_to_str(::romp::ostream_p& out, const anon0x1_t& val) noexcept { out << "[\n" << out.indent(); for (size_t i=0; i<val.size(); ++i) { out << out.indentation() << "[" << (i + (VALUE_C(0))) << "]: "<< anon0x0_t_to_str(out,val.data[i]) << '\n'; } out << out.dedent() << out.indentation() << ']'; return ::romp::S_VOID;}
...
#undef pong
    ...
    bool __Property__Only_one_ball_in_play_(void) const {
      using namespace ::__type__;
      try {
        return this->__rw__->invariant_handler(({ bool res_0 = true; for (int p = ((int)(0)); p <= ((int)(1)); p++) { res_0&= (!((!((((Players).data[((p)) - int_VALUE_C(0)]).hasball) && (((Players).data[((p)) - int_VALUE_C(0)]).gotball))) && ((((Players).data[((p)) - int_VALUE_C(0)]).hasball) || (((Players).data[((p)) - int_VALUE_C(0)]).gotball))) || ({ bool res_1 = true; for (int q = ((int)(0)); q <= ((int)(1)); q++) { res_1&= (!((((Players).data[((q)) - int_VALUE_C(0)]).hasball) || (((Players).data[((q)) - int_VALUE_C(0)]).gotball)) || ((p) == (q))); } res_1; })); } res_0; }),0u);
...
#ifdef __ROMP__SIMPLE_TRACE
template<class O> const ::romp::stream_void& __Model___to_json(::romp::ojstream<O>& json, const __Model__& val) noexcept { using namespace ::__type__; json  << anon0x1_t_to_json(json,val.Players); return ::romp::S_VOID; }
#else
template<class O> const ::romp::stream_void& __Model___to_json(::romp::ojstream<O>& json, const __Model__& val) noexcept { using namespace ::__type__; json << "{\"$type\":\"record-value\",\"type\":\"__Model__\",\"fields\":[" << "{\"$type\":\"kv-pair\",\"key\":\"Players\",\"value\":" << anon0x1_t_to_json(json,val.Players)<< "}]}"; return ::romp::S_VOID;}
#endif
const ::romp::stream_void& __Model___to_str(::romp::ostream_p& out, const __Model__& val) noexcept { using namespace ::__type__; out << "{\n" << out.indent() << out.indentation() << "Players " << ((::romp::OPTIONS.result_show_type) ? ": Array[player_t] of anon_record_t =" : ":=") << ' ' << anon0x1_t_to_str(out,val.Players) << '\n' << out.dedent()<< out.indentation() << '}'; return ::romp::S_VOID;}
}
namespace romp {
#ifdef __ROMP__SIMPLE_TRACE
...
#define _ROMP_STATESPACE_COUNT_str "729"
...
#define _ROMP_STARTSTATE_INFO_LEN (1ul)
  ...
  const ::romp::StartStateInfo STARTSTATE_INFOS[_ROMP_STARTSTATE_INFO_LEN] = {{"startstate2",{{61,5},{66,8}},"{\"$type\":\"startstate\",\"label\":\"startstate2\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/pingpong.m\",\"start\":[61,5],\"end\":[66,8]}"},};
...
#define _ROMP_RULESETS_LEN (3ul)
  ...
  const ::romp::RuleInfo RULE_SET_INFOS[_ROMP_RULESETS_LEN] = {{"Get_ball",{{39,5},{45,8}},"{\"$type\":\"rule\",\"expr\":\"ping.gotball\",\"label\":\"Get_ball\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/pingpong.m\",\"start\":[39,5],\"end\":[45,8]}"},{"Keep_ball",{{47,5},{51,8}},"{\"$type\":\"rule\",\"expr\":\"ping.hasball\",\"label\":\"Keep_ball\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/pingpong.m\",\"start\":[47,5],\"end\":[51,8]}"},{"Pass_ball",{{53,5},{59,8}},"{\"$type\":\"rule\",\"expr\":\"ping.hasball\",\"label\":\"Pass_ball\",\"loc\":{\"$type\":\"location\",\"file\":\"/mnt/g/My Drive/School/UofU/proj/romp/rumur-romp/romp/tests/pingpong.m\",\"start\":[53,5],\"end\":[59,8]}"},};
...
#define _ROMP_STARTSTATES_LEN (2ul) // the number of property rules (after ruleset expansion) in the model
  ...
  // All of the rule sets in one place
const ::romp::StartState STARTSTATES[_ROMP_STARTSTATES_LEN] = {{__StartState__startstate2__0,::__info__::STARTSTATE_INFOS[0],0,"[{\"$type\":\"kv-pair\",\"key\":\"p\",\"value\":{\"$type\":\"range-value\",\"type\":\"player_t\",\"value\":0}}]","p:=0"}, {__StartState__startstate2__1,::__info__::STARTSTATE_INFOS[0],1,"[{\"$type\":\"kv-pair\",\"key\":\"p\",\"value\":{\"$type\":\"range-value\",\"type\":\"player_t\",\"value\":1}}]","p:=1"}};
